...
let ManagedObjectSymbol, ElementSymbol, ControlSymbol;
function interestingBaseClassForSymbol(typeChecker, symbol) {
    if (!ManagedObjectSymbol) {
        ...
        const managedObjectModuleDeclaration = typeChecker
            ...
            .declarations[0];
        const managedObjectClassDeclaration = managedObjectModuleDeclaration.body.statements.filter((s) => { var _a; return ts.isClassDeclaration(s) && ((_a = s.name) === null || _a === void 0 ? void 0 : _a.text) === "ManagedObject"; })[0];
        ManagedObjectSymbol = typeChecker.getSymbolAtLocation(managedObjectClassDeclaration.name);
        const elementModuleDeclaration = typeChecker
            .getAmbientModules()
            .filter((m) => m.name === '"sap/ui/core/Element"')[0]
            .declarations[0];
        const elementClassDeclaration = elementModuleDeclaration.body.statements.filter((s) => { var _a; return ts.isClassDeclaration(s) && ((_a = s.name) === null || _a === void 0 ? void 0 : _a.text) === "UI5Element"; })[0];
        ElementSymbol = typeChecker.getSymbolAtLocation(elementClassDeclaration.name);
        const controlModuleDeclaration = typeChecker
            .getAmbientModules()
            .filter((m) => m.name === '"sap/ui/core/Control"')[0]
            .declarations[0];
        const controlClassDeclaration = controlModuleDeclaration.body.statements.filter((s) => { var _a; return ts.isClassDeclaration(s) && ((_a = s.name) === null || _a === void 0 ? void 0 : _a.text) === "Control"; })[0];
        ControlSymbol = typeChecker.getSymbolAtLocation(controlClassDeclaration.name);
    }
...
function generateInterface({ sourceFile, className, settingsTypeFullName, interestingBaseClass, constructorSignaturesAvailable, metadata, }, allKnownGlobals, options) {
    ...
    const objectLiteralExpression = metadata[0]
        .initializer; // the entire object literal defining all the control metadata
    ...
    const ast = buildAST(classInfo, sourceFile.fileName, constructorSignaturesAvailable, moduleName, settingsTypeFullName, allKnownGlobals, options);
    if (!ast) {
...
function buildAST(classInfo, classFileName, constructorSignaturesAvailable, moduleName, settingsTypeFullName, allKnownGlobals, options) {
    ...
    if (requiredImports.selfIsUsed) {
        ...
        ts.addSyntheticLeadingComment(module2, ts.SyntaxKind.SingleLineCommentTrivia, " this duplicate interface without export is needed to avoid \"Cannot find name '" +
            classInfo.name +
